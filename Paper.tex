...
\author{
    ...
    gavin@parity.io \\
    Translated by Mr.P - \url{https://t.me/popovitsch}
}
\begin{document}
...
Парадигма блокчейн в связке c криптографически защищенными транзакциями продемонстрировала свою эффективность(полезность) в ряде проектов, один из самых внушительных примеров - Биткойн. Каждый такой проект может быть представлен как простое приложение в децентрализованном, но едином(одноэлементном), вычислительном ресурсе. Мы можем называть эту парадигму транзакционной одноэлементной(единой) машиной с разделенным(распределенным) состоянием.

Ethereum реализует эту парадигму в обобщенном виде. Кроме того, он(Ethereum) предоставляет множество таких ресурсов, каждый из которых со своим отдельным состоянием и исполняемым кодом, и все они способны взаимодействовать друг с другом через платформу передачи сообщений. Мы обсуждаем его дизайн, вопросы реализации, возможности, которые он предоставляет, будущие проблемы, которые мы предвидим.
\end{abstract}
...
\section{
  \selectlanguage{russian}
  Введение}\label{sec:introduction}
\selectlanguage{russian}
С повсеместным подключением к интернету в большинстве мест мира, глобальная передача информации стала невероятно дешёвой. Технологические решения, как Биткойн, благодаря силе дефолта, продемонстрировало механизм консенсуса и добровольное соблюдение общественного договора, и показало, что можно использовать  интернет для децентрализованной передачи ценностей, которую можно проводить по всему миру и практически бесплатно. Эту систему можно описать, как стейт-машина(конечный автомат) с очень специализированной версией криптографической безопасности основанной на транзакциях. Последующие системы, такие как Namecoin, адаптировали оригинальное "валютное приложение" в другое приложение, хотя и довольно упрощенно.

Ethereum это проект, который пытается построить обобщенную технологию; технологию на которой могут быть построены все концепции стейт-машин(конечных автоматов), основанных на транзакциях. Более того, это нацелено на то, чтобы проедоставить конечным разработчикам интегрированную сквозную систему для разработки ПО на основе до сих пор неизведанной вычислительно парадигмы: доверительная вычислительная платформа отправки сообщений.
\subsection{
  \selectlanguage{russian}
  Движущие факторы
  } \label{ch:driving}

У этого проекта много целей; одна из основных целей это облегчить транзакции между согласными лицами, которые в противном случае не имели бы возможности доверять друг другу. Это может быть из-за географического разделения, сложность взаимодействовия, или возможно несовместимость, некомпетентность, нежелание, дороговизна, неопределенность, неудобство, или коррупция существующих правовых систем.  Указав систему изменения состояния через богатый и однозначный язык, кроме того, разрабатывая такую систему, в который мы можем ожидать достижения соглашений автономным путем, мы готовы выделить средства на поддержку.

Сделки в этой предложенной системе будут иметь несколько атрибутов, которые редко встречаются в реальном мире. Неподкупность суждений, которую не так то просто найти, естественно исходит от незаинтересованного алгоритмичесикого интерпретатора. Прозрачность(открытость), или возможность увидеть как именно возникло состояние или решение(суждение) через журнал транзакций и правил или инструкционные коды, никогда не бывает в системх, основанных на человеке. Ведь естественный язык неизбежно имеет неопределенности, информации часто не хватает, да и избавиться от старых предрассудков трудно.

В целом, мы хотим предоставить такую систему, в которой пользователи могли бы быть уверены, что независимо от того, с какими людьми, системами или организациями они взаимодействуют, они это могут делать с абсолютной уверенностью в ожидаемых результатах и том как это всё произойдет.


\subsection{Предыдущая работа} \label{ch:previous}

\cite{buterin2013ethereum} первым предложил ядро этой работы в конце Ноября, 2013. Хотя в настоящее время во многом эволюционировал, ключевая функцональность блокчейна с Тьюринг-полным языком и фактически неограниченные возможности хранения межтранзакционных данных остаются неизменными.

\cite{dwork92pricingvia} предоставил первую работу по использованию криптографического доказательства вычислительных затрат (``proof-of-work'') в качестве средства передачи ценностного сигнала через Интернет. Этот сигнал был использован здесь в качестве механизма сдерживающего спам, а не как валюты, но критически продемонстрировал потенциал передачи \textit{сильных экономических сигналов} по основным канала данных, позволяющее получателю сделать физическое утверждение без необходимости полагаться на \textit{доверие}. \cite{back2002hashcash} позднее произвел систему в аналогичном ключе.

Первый пример использования proof-of-work в качестве сильного экономического сигнала для обеспечения безопасности валюты был приведен \cite{vishnumurthy03karma:a}. В данном случае токен использовался для поддержания пиринговой торговли файлами, предоставляя ``потребителям'' возможность осуществлять микроплатежи ``поставщикам'' за их услуги. Модель безопасности, обеспечиваемая proof-of-work, была дополнена цифровыми пописями и бухгалтерской книгой, чтобы гарантировать истинность исторических записей и что злоумышленники не смогут подделать платежи или несправедливо пожаловаться на предоставление услуг. Пять лет спустя \cite{nakamoto2008bitcoin} представил еще один токен proof-of-work-secured (защищенный доказательством работы). Плодом этого проекта - Биткойн, который стал первым широко распространенным глобальной децентрализованной системой(книгой) учета транзакций.
Другие проекты, основанные на успехе Биткойна; альткойны предоставили множество других валют путем изменения протопа. Одни из самых известных это Litecoin и Primecoin, обсуждаемые \cite{sprankel2013technical}. Другие проекты стремились взять основной механизм протокола и перепрофилировать его; \cite{aron2012bitcoin} обсуждает, например, проект Namecoin, целью которого было предоставить децентрализованную систему разрешения имен.
Есть проекты, которые по-прежнему основаны на самой сети Биткойн, используют большое кол-во ценности, помещенной в систему, и огромное кол-во вычислений, которые входят в систему консенсуса. Проект Mastercoin, впервые предложенный \cite{mastercoin2013willett}, нацелен на разработку более богатого протокола, включающего множество дополнительных высокоуровневых улучшений поверх Биткойн протокола за счет использования ряда вспомогательных частей основного протокола. Проект Coloured Coins, предложенный \cite{colouredcoins2012rosenfeld}, использует схожую, но более упрощенную стратегию, приукрашивая правила транзакция с целью нарушить взаимозаменяемость базовой валюты Биткойн и позволить создавать и отслеживать токены с помощью специального программного обеспечения с поддержкой протокола "хромо-кошелька".
Была проведена дополнительная работа, чтобы отбросить основы децентрализации; Ripple, 
